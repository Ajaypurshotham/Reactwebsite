...
const makePayment = async()=>{
  const stripe = await loadStripe("pk_test_51OSJCQSAo0roDB5gJ6BfrttcfwE1RwYpyJopCN4RUubKovCjjPhhb69DRutEPlJUpujTVd7IMuw052oJVezShKLs0075bYikNa");
const body ={
  products:carts
}
const headers = {
  ...
  <div className="card">
    <div className="card-header bg-dark p-3">
      <div className="card-header-flex">
        <h5 className='text-white m-0'>My Cart{carts.length > 0 ? `(${carts.length})`:""}</h5>
        {
...
><i className='fa fa-trash-alt mr-2'></i></button>
  ...
  <tfoot>
    <tr>
      <th>&nbsp;</th>
      <th colSpan={2}>&nbsp;</th>
      <th>Item in Cart <span className='m1-2 mr-2'>:</span><span className='text-danger'>{totalquantity}</span></th>
      <th className='text-right'>Total Price <span className='m1-2 mr-2'>:</span><span className='text-danger'>{totalprice}</span></th>
